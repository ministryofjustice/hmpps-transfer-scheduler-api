...
class SyncDeleteTransferIntTest(
  ...
  fun `204 no content - can delete a transfer with movement - movement becomes unscheduled`() {
    ...
    assertThat(updated.reason.code).isEqualTo(movement.reasonCode)
    assertThat(updated.destinationCode).isEqualTo(movement.destinationCode)
    assertThat(updated.logistics?.code).isEqualTo(movement.logisticsCode)
